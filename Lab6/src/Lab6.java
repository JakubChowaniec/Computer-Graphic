...
public class Lab6 extends GLJPanel implements GLEventListener {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        triangle_3D(gl2, 6, 3, 11);
    } // end display()


    public void triangle_3D(GL2 gl2, int height, int r, int angle) {
        double angleSum = 0;
        double oneAngle = 2 * Math.PI / angle;

        gl2.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_AMBIENT_AND_DIFFUSE, materials[15], 10);
        gl2.glBegin(gl2.GL_TRIANGLE_FAN);
        gl2.glVertex3d(0, height, 0);
        for (int i = 0; i <= angle; i++) {
            gl2.glVertex3d((Math.sin(angleSum) * r), 0, (Math.cos(angleSum) * r));
            angleSum +=oneAngle;
        }
        gl2.glEnd();
    }
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        gl2.glLightModeli(GL2.GL_LIGHT_MODEL_LOCAL_VIEWER,5);
        float[] ambientLevel = {0.4f,0.4f,0.4f,1};
        gl2.glLightModeli(GL2.GL_LIGHT_MODEL_AMBIENT,3);
    }
